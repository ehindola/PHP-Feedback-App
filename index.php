...
<?php
    ...
    if(isset($_POST['submit'])) {
      ...
      if(empty($_POST['name'])) {
       ...
       // Validate body
       if(empty($_POST['body'])) {
        $bodyErr = 'Feddback is required';
      } else {
        $body = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
      }

      if(empty($emailErr) && empty($emailErr) && empty($bodyErr)) {
        //Add to database
...
?>
    ...
    <img src="/feedback/img/new logo-01.png" class="w-25 mb-3" alt="">
    <h2>Feedback</h2>
    <p class="lead text-center">Leave feedback for ehindola dev</p>
